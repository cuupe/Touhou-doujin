...
namespace Engine::Core::Components {
	...
	void AnimationComponent::Update(float d_t, Context& ctx)
	{
		...
		AnimationFrame current_frame = current_animation->GetInterpolatedFrame(animation_timer);
		sprite->SetRect(current_frame.rect);
		sprite->SetScale(current_frame.scale);
		sprite->SetRotation(current_frame.rotation);
		sprite->SetColorMod(current_frame.color_mod);
		auto trans = owner->GetComponent<Engine::Core::Components::TransformComponent>();
		trans->Translate(current_frame.offset);
