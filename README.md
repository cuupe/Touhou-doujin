...
1.由于多线程代码和`Directx11`处理的不是很好，偶尔开启游戏会出现贴图丢失或卡死，此属于正常现象。
2.尽量避免使用窗口控件关闭窗口，容易线程冲突，尤其是处于关卡中，绝对不要直接关闭窗口。
3.请不要动着色器，这不是开放式渲染引擎，所以不懂如何修改着色器请不要动，否则黑屏或卡死后果自负。
4.你可以修改的地方是资源文件，如果你愿意研究的话可以添加你自己的敌人或者子弹数据之类的，但是注意，
所有数据格式必须与当前文件格式保持一致，否则出错。
...
| `ClearBullets(turn)`                                         | bool                              | false    | -                                                            | 无     | true=有凋落物false=无凋落物          |
